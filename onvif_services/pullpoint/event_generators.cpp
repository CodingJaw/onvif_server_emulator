...
namespace osrv
{
    namespace event
    {
        ExternalToggleEventGenerator::ExternalToggleEventGenerator(const std::string& source_token,
                const std::string& data_name, const std::string& topic, boost::asio::io_context& io_context,
                const ILogger& logger, bool initial_state)
                : IEventGenerator(0, topic, io_context, logger)
        ...
        void ExternalToggleEventGenerator::Trigger(bool state)
        {
                TRACE_LOG(logger_);
                ...
                if (!source_token_.empty())
                        nm.source_item_descriptions.push_back({"Source", source_token_});
                nm.data_name = data_name_;
                nm.data_value = state_ ? "true" : "false";

                event_signal_(nm);
        }

        std::deque<NotificationMessage> ExternalToggleEventGenerator::GenerateSynchronizationEvent() const
        {
                TRACE_LOG(logger_);
